...
모바일-to-모바일 전송은 모바일 앱도 서버 롤을 가져야 합니다. 모바일 앱에 `10분 열기` 단기 수신 모드를 넣어, 앱이 활성 상태일 때 임시 gRPC 서버와 SSDP responder를 열고 다른 모바일이 서버 탐색 후 `SendFile`로 전송할 수 있게 시작했습니다. 이번 단계에서는 모바일 수신 승인 팝업, UUID 기반 항상 허용 목록, 수신 진행률 UI/Android 알림까지 연결했습니다. Android의 장시간 수신은 다음 단계에서 foreground service 위로 올리고, iOS는 장시간 백그라운드 서버 대기 제약 때문에 앱 활성 상태 중심 수신부터 안정화합니다.
