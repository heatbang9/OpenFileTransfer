...
- [OpenFileTransferWeb](https://github.com/heatbang9/OpenFileTransferWeb): Vercel 정적 배포용 WebRTC 브라우저 파일 전송 앱

## 목표 구조

- Mobile: Flutter Client
- PC: Server + Client
- Web: 설치 없는 WebRTC P2P Client
...
- Vercel 웹 앱 기반 WebRTC P2P 파일 전송, Nearby 방 디바이스 목록, QR 스캔/공유 링크, TURN 설정/테스트, PWA 캐시, 실패 큐 재시도, File System Access/OPFS 저장, 청크 단위 이어받기, 전송 이력, 선택 AES-GCM 암호화
...
- 모바일 앱은 `10분 열기` 단기 수신 모드에서 임시 gRPC 서버/SSDP responder를 열고, 다른 모바일의 `SendFile`을 받을 수 있습니다.
- 모바일-to-모바일 수신은 상대 UUID/이름을 저장하고, 미승인 기기에는 `이번만 허용`/`항상 허용`/`거부` 팝업을 띄웁니다. 항상 허용된 UUID는 이후 바로 수신하거나 수신함 파일을 내려받을 수 있습니다.
- 모바일-to-모바일 수신 진행률은 앱 UI와 Android foreground service 알림에 표시됩니다.
- 모바일 수신 모드가 켜져 있는 동안 Android foreground service 알림을 유지해 백그라운드 수신 대기를 보강합니다.
- 모바일 앱도 발견된 여러 PC/모바일 서버를 체크하고 같은 파일을 순차 전송하는 1:N 큐와 대상별 성공/실패 결과 UI를 갖습니다.

## 모바일-to-모바일 방향

모바일-to-모바일 전송은 모바일 앱도 서버 롤을 가져야 합니다. 모바일 앱에 `10분 열기` 단기 수신 모드를 넣어, 앱이 활성 상태일 때 임시 gRPC 서버와 SSDP responder를 열고 다른 모바일이 서버 탐색 후 `SendFile`로 전송할 수 있게 시작했습니다. 이번 단계에서는 모바일 수신 승인 팝업, UUID 기반 항상 허용 목록, 수신 진행률 UI/Android 알림, 모바일 1:N 순차 전송 큐까지 연결했습니다. Android의 장시간 수신은 다음 단계에서 foreground service 위로 올리고, iOS는 장시간 백그라운드 서버 대기 제약 때문에 앱 활성 상태 중심 수신부터 안정화합니다.
