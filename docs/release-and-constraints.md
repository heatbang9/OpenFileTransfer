...
- Web: Vercel 배포용 WebRTC 브라우저 앱, Runtime Cache 기반 Nearby 방, QR 스캔/공유 링크, 여러 파일 큐, File System Access/OPFS 저장, 청크 단위 이어받기, TURN 설정/테스트, PWA 캐시, 실패 큐 재시도, 전송 이력, 선택 AES-GCM 암호화까지 구현했습니다.
