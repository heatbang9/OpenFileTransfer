...
OpenFileTransfer의 기본 아이콘은 이미지 생성 모델로 만든 민트색 파일 전송 아이콘이며, 로컬 네트워크에서 파일을 주고받는 흐름을 표현합니다.

- 둥근 사각형: 앱 아이콘과 안정적인 연결
- 태극형 원형 흐름: 양방향 전송과 상호 신뢰
- 상단 화살표: 우측에서 좌측으로 흐르는 수신 방향
- 하단 화살표: 좌측에서 우측으로 흐르는 송신 방향
- 중앙 파일 문서: 실제 파일 전달 앱이라는 의미
...
- PC 앱: Electron renderer에서 같은 PNG 아이콘과 민트/틸 버튼 스타일을 사용합니다.
- 모바일 앱: Flutter 앱 내부 로고와 launcher icon이 같은 PNG 아이콘을 사용합니다.
- 앱 아이콘: `assets/brand/openfiletransfer-icon-generated.png`를 원본으로 보관하고, `openfiletransfer-icon-1024.png`를 launcher/window icon 원본으로 사용합니다.
